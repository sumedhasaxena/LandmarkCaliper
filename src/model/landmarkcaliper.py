...
class LandmarkCaliper(object):
    ...
    def init_model(self) -> None: 
        ...
        if model_name == "HAND_GEOM":
            ...
            self.measurement_model = ScalingDistanceCalculator(landmark_mapping_file, hand_measurement_file, a4_size = a4_size)
    ...
    def show_measurement_details(self, show_hand = True) -> None:
        ...
        print(f'Hand Type: {hand_type}, Length: {round(max_length, 2)} cm b/w {max_landmarks[0]} to {max_landmarks[1]}\n')

        self.show_hand_landmark_image()
    ...
    @beartype
    def show_hand_landmark_image(self, show_pixel_distance = False, fig_size = (10, 15)) -> None:

        image = self.draw_hand_landmarks(show_pixel_distance = show_pixel_distance)
        ...
        plt.imshow(image)
        plt.title(f'Landmark measurements for {image_name} (cm): {hand_type} Hand, Length: {round(max_length, 1)} cm b/w {max_landmarks[0]} to {max_landmarks[1]}')
    ...
    @beartype
    def save_measurements(self, hand_measurements = True, landmark_measurements = True, landmark_display = True, contour_display = True, output_dir = None) -> list[str]:
        ...
        if landmark_display:
            ...
            output_file3 = get_landmark_display_file_name(self.image_file, hand_type, output_dir = output_dir)

            logger.info(f'saving landmark display image to: {output_file3}')

            # save landmark display
            self.save_hand_landmark_image(output_file3)
    ...
    @beartype
    def save_hand_landmark_image(self, file_name : str) -> None:
    ...
    @beartype
    def draw_hand_landmarks(self, show_pixel_distance = False) -> np.ndarray:
